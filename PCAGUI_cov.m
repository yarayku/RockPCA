...
Tdata = TData;
Ldata = LData;
[TPC,Tscore,~,~,Texplained] = pca(cov(Tdata));
[LPC,Lscore,~,~,Lexplained] = pca(cov(Ldata));
% pre-load value
TatPC = Tdata*TPC(:,1:3);
LatPC = Ldata*TPC(:,1:3);
...
set(hTaxes1,'xlim',[0 11],'ylim',[0 100])
...
Tb = bar([1:10],[abs(TPC(1,1:3));abs(TPC(2,1:3));abs(TPC(3,1:3));...
    abs(TPC(4,1:3));abs(TPC(5,1:3));abs(TPC(6,1:3));abs(TPC(7,1:3));...
    abs(TPC(8,1:3));abs(TPC(9,1:3));abs(TPC(10,1:3))]);
set(hTaxes2,'xtick',[1:10],'XTickLabel',title,'XLim',[0 11],'YLim',[0 1])
set(Tb,'BarWidth',1)
...
set(hLaxes1,'xlim',[0 11],'ylim',[0 100])
xlabel('Principal Component (Eigenvector)')
ylabel('Variance Explained %')
% axes2
hLaxes2 = axes('Parent',hLpanel,'Units','normalized','Position',Lpaxes2,...
    'NextPlot','add','ALimMode','manual','tag','Lax2');
Lb = bar([1:10],[abs(LPC(1,1:3));abs(LPC(2,1:3));abs(LPC(3,1:3));...
    abs(LPC(4,1:3));abs(LPC(5,1:3));abs(LPC(6,1:3));abs(LPC(7,1:3));...
    abs(LPC(8,1:3));abs(LPC(9,1:3));abs(LPC(10,1:3))]);
set(hLaxes2,'xtick',[1:10],'XTickLabel',title,'XLim',[0 11],'YLim',[0 1])
ylabel('Loading')
set(Lb,'BarWidth',1)
pattern = legend('PC1','PC2','PC3');
set(pattern,'FontSize',6)
